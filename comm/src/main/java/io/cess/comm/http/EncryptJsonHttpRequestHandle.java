...
//        json = Base64.getEncoder().encodeToString(json.getBytes());
        json = io.cess.util.Base64.encode(json);
//        try {
...
//			byte[] tmpBs = Base64.getDecoder().decode(resp);
			...
			resp = io.cess.util.Base64.decode(resp,"utf-8");
		} catch (Throwable e1) {
			e1.printStackTrace();
...
//			error.setCode(-3);
		...
		Error error = null;
		try{
//			obj  = io.cess.util.json.JSONUtil.deserialize(resp);
//			ResultData resultData = (ResultData) io.cess.util.json.JSONUtil.deserialize(obj,ResultData.class);
...
//				obj = io.cess.util.json.JSONUtil.deserialize(map.get("result"), pack.getRespType());
