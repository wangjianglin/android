...
import io.cess.core.gallery.PauseOnScrollListener;
...
import io.cess.demo.gallery.listener.UILPauseOnScrollListener;
...
public class GalleryActivity extends ViewActivity {
    ...
    @ViewById(R.id.rb_uil)
    RadioButton mRbUil;
    @ViewById(R.id.rb_glide)
    RadioButton mRbGlide;
    @ViewById(R.id.rb_picasso)
    RadioButton mRbPicasso;
    @ViewById(R.id.rb_single_select)
    RadioButton mRbSingleSelect;
    @ViewById(R.id.rb_muti_select)
    RadioButton mRbMutiSelect;
    @ViewById(R.id.et_max_size)
    EditText mEtMaxSize;
    @ViewById(R.id.btn_open_gallery)
    Button mBtnOpenGallery;
    @ViewById(R.id.lv_photo)
    HorizontalListView mLvPhoto;
    @ViewById(R.id.cb_edit)
    CheckBox mCbEdit;
    @ViewById(R.id.cb_crop)
    CheckBox mCbCrop;
    @ViewById(R.id.cb_rotate)
    CheckBox mCbRotate;
    @ViewById(R.id.cb_show_camera)
    CheckBox mCbShowCamera;
    @ViewById(R.id.ll_max_size)
    LinearLayout mLlMaxSize;
    @ViewById(R.id.ll_edit)
    LinearLayout mLlEdit;
    @ViewById(R.id.rb_xutils)
    RadioButton mRbXutils;
    @ViewById(R.id.rb_xutils3)
    RadioButton mRbXutils3;
    @ViewById(R.id.et_crop_width)
    EditText mEtCropWidth;
    @ViewById(R.id.et_crop_height)
    EditText mEtCropHeight;
    @ViewById(R.id.ll_crop_size)
    LinearLayout mLlCropSize;
    @ViewById(R.id.cb_crop_square)
    CheckBox mCbCropSquare;
    @ViewById(R.id.rb_theme_default)
    RadioButton mRbThemeDefault;
    @ViewById(R.id.rb_theme_dark)
    RadioButton mRbThemeDark;
    @ViewById(R.id.rb_theme_cyan)
    RadioButton mRbThemeCyan;
    @ViewById(R.id.rb_theme_orange)
    RadioButton mRbThemeOrange;
    @ViewById(R.id.rb_theme_green)
    RadioButton mRbThemeGreen;
    @ViewById(R.id.rb_theme_teal)
    RadioButton mRbThemeTeal;
    @ViewById(R.id.rb_theme_custom)
    RadioButton mRbThemeCustom;
    @ViewById(R.id.cb_crop_replace_source)
    CheckBox mCbCropReplaceSource;
    @ViewById(R.id.cb_rotate_replace_source)
    CheckBox mCbRotateReplaceSource;
    @ViewById(R.id.cb_open_force_crop)
    CheckBox mCbOpenForceCrop;
    @ViewById(R.id.cb_open_force_crop_edit)
    CheckBox mCbOpenForceCropEdit;
    @ViewById(R.id.ll_force_crop)
    LinearLayout mLlForceCrop;
    @ViewById(R.id.toolbar)
    Toolbar mToolbar;
    @ViewById(R.id.rb_fresco)
    RadioButton mRbFresco;
    @ViewById(R.id.cb_preview)
    CheckBox mCbPreview;
    @ViewById(R.id.cb_no_animation)
...
//        ButterKnife.bind(this);
        ...
        mOpenGallery.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                io.cess.core.gallery.ImageLoader imageLoader = null;
                PauseOnScrollListener pauseOnScrollListener = null;
                if (mRbUil.isChecked()) {
                    imageLoader = new UILImageLoader();
                    pauseOnScrollListener = new UILPauseOnScrollListener(false, true);
                } else if (mRbXutils.isChecked()) {
                    //imageLoader = new XUtils2ImageLoader(GalleryActivity.this);
                    ...
                    //imageLoader = new FrescoImageLoader(GalleryActivity.this);
                } else {
                ...
                CoreConfig coreConfig = new CoreConfig.Builder(GalleryActivity.this, imageLoader, themeConfig)
                ...
                ActionSheet.createBuilder(GalleryActivity.this, getSupportFragmentManager())
                                ...
                                switch (index) {
                                    ...
                                    case 2:
                                        if (new File(path).exists()) {
                                            ...
                                            Toast.makeText(GalleryActivity.this, "图片不存在", Toast.LENGTH_SHORT).show();
                                        }
                                        break;
                                    case 3:
                                        if (new File(path).exists()) {
...
//        ImageLoaderPipelineConfig config = ImagePipelineConfig.newBuilder(this)
