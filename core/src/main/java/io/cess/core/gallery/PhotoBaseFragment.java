...
import io.cess.core.gallery.utils.*;
...
//        ActivityManager.getActivityManager().finishActivity(this);
    ...
    protected void takePhotoAction() {
        if (!DeviceUtils.existSDCard()) {
            String errormsg = getString(R.string.io_cess_core_grallery_empty_sdcard);
            toast(errormsg);
            if (mTakePhotoAction) {
        ...
        if ( requestCode == GalleryFinal.TAKE_REQUEST_CODE ) {
            if (resultCode == RESULT_OK && mTakePhotoUri != null) {
                ...
                if (new File(path).exists()) {
                    ...
                    info.setPhotoId(io.cess.core.gallery.utils.Utils.getRandom(10000, 99999));
    ...
    private void takePhotoFailure() {
        String errormsg = getString(R.string.io_cess_core_grallery_take_photo_fail);
        if (mTakePhotoAction) {
            resultFailure(errormsg, true);
        } else {
            toast(errormsg);
    ...
    protected void resultData(ArrayList<PhotoInfo> photoList) {
        ...
        if (callback != null) {
            if ( photoList != null && photoList.size() > 0 ) {
                ...
                callback.onHanlderFailure(requestCode, getString(R.string.io_cess_core_grallery_photo_list_empty));
            }
        }
//        finishGalleryFinalPage();
    }
