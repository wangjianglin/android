...
public class FitChart extends View {
    ...
    static float START_ANGLE = -90;
    static final float DEFAULT_START_ANGLE = -90;
    ...
    private float preStartAngle = 0;
    private FitChartValue chartValue = null;
    public void setValue(float value) {
        if (chartValue == null) {
//            START_ANGLE = DEFAULT_START_ANGLE;
            START_ANGLE = 0;
            chartValues.clear();
            chartValue = new FitChartValue(value, valueStrokeColor);
//            preStartAngle = START_ANGLE;
            chartValues.add(chartValue);
        }else{
            START_ANGLE = preStartAngle;
        }

        chartValue.setValue(value);
        chartValue.setPaint(buildPaintForValue());
        chartValue.setStartAngle(START_ANGLE);
//        preStartAngle = calculateSweepAngle(value);
        preStartAngle = calculateSweepAngle(value);
        chartValue.setSweepAngle(preStartAngle);
...
//        System.out.println("animationProgress:"+animationProgress);
    ...
    private float calculateAnimationSeek() {
        ...
        //return ((maxSweepAngle-START_ANGLE) * animationProgress) + START_ANGLE;
    }
