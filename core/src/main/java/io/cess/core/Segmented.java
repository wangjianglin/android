...
@ResCls(value=io.cess.core.R.class)
...
public class Segmented extends ResView {
	...
	private void resetViews(){
		...
		if(items.size() == 0){
			viewItem = LayoutInflater.from(this.getContext()).inflate(R.layout.io_cess_core_segmented_item, layout, false);
		...
		for(int n=0;n<items.size();n++){
			...
			if(n == 0) {
				viewItem = LayoutInflater.from(this.getContext()).inflate(R.layout.io_cess_core_segmented_item_left, layout, false);
...
//				drawable.se
				...
				viewItem = LayoutInflater.from(this.getContext()).inflate(R.layout.io_cess_core_segmented_item_right, layout, false);
//				drawable.setCornerRadii(new float[]{roundRadius, roundRadius, 0, 0, 0, 0, roundRadius, roundRadius});
				...
				viewItem = LayoutInflater.from(this.getContext()).inflate(R.layout.io_cess_core_segmented_item_middle, layout, false);
...
//			LayerDrawable d = (LayerDrawable) this.getContext().getResources().getDrawable(R.drawable.io_cess_core_form_section_separate);
////			viewItem.setBackgroundDrawable(this.getContext().getResources().getDrawable(R.drawable.io_cess_core_form_section_separate));
...
//			GradientDrawable dd = (GradientDrawable) d.getDrawable(1);
	...
	public void setSelectedIndex(int index){
		...
		if(index >=0 && index < layout.getChildCount()) {
			...
			((TextView)(layout.getChildAt(index).findViewById(R.id.io_cess_core_segmented_text_view_id))).setTextColor(this.textSelectedColor);
		}
		if(preIndex >=0 && preIndex < layout.getChildCount()) {
			...
			((TextView)(layout.getChildAt(preIndex).findViewById(R.id.io_cess_core_segmented_text_view_id))).setTextColor(this.textColor);
		}
	...
	private void setViewItem(View viewItem,final int index, final String item){
		TextView textView = (TextView) viewItem.findViewById(R.id.io_cess_core_segmented_text_view_id);
