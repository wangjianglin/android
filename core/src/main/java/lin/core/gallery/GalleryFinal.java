...
public class GalleryFinal {
    ...
    public static void openGallerySingle(int requestCode, OnHanlderResultCallback callback) {
        ...
        if (config != null) {
            ...
            if(callback != null) {
                callback.onHanlderFailure(requestCode, mCoreConfig.getContext().getString(R.string.lin_core_grallery_open_gallery_fail));
            }
    ...
    public static void openGallerySingle(int requestCode, FunctionConfig config, OnHanlderResultCallback callback) {
        ...
        if (!DeviceUtils.existSDCard()) {
            Toast.makeText(mCoreConfig.getContext(), R.string.lin_core_grallery_empty_sdcard, Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    public static void openGalleryMuti(int requestCode, FunctionConfig config, OnHanlderResultCallback callback) {
        ...
        if ( config.getMaxSize() <= 0) {
            if(callback != null){
                callback.onHanlderFailure(requestCode, mCoreConfig.getContext().getString(R.string.lin_core_grallery_maxsize_zero_tip));
            }
            return;
        }
        ...
        if (config.getSelectedList() != null && config.getSelectedList().size() > config.getMaxSize()) {
            if(callback != null){
                callback.onHanlderFailure(requestCode, mCoreConfig.getContext().getString(R.string.lin_core_grallery_select_max_tips));
            }
            return;
        }
